...
-- //just some fancy teleportation vfx
...
local function timeToMove()
    ...
    local dest
    repeat
        dest = table.choice(creeperDest) -- //random choice of cell to travel to
    until tes3.getCell {
        id = dest.cell
    } ~= creeperRef.cell
...
local function onInfoGetText(e)
    ...
    if e.info.type == tes3.dialogueType.greeting then
        if not e:loadOriginalText():match("Caldera") then
            return
        end
        e.text = e:loadOriginalText():gsub("Caldera", creeperRef.cell.name:match("[^,]+"))
    end
end
...
local function latestRumors(e)
    ...
    if math.random(100) < 20 and e.info.type == tes3.dialogueType.topic and e.info.objectFlags < 12 then
